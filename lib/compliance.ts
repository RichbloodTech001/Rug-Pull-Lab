...
export type ComplianceProfile = {
  ...
  kyb: VerificationStatus;
  sanctions: "CLEAR" | "MATCH" | "PENDING";
  pep: "CLEAR" | "MATCH" | "PENDING";
  sourceOfFunds: VerificationStatus;
  riskTier: RiskTier;
};
...
export type TransactionScreenInput = {
  amountMinor: bigint;
  currency: string;
  destination?: string;
  country?: string;
...
export function evaluateTransaction(profile: ComplianceProfile, input: TransactionScreenInput): ComplianceDecision {
  ...
  if (profile.sanctions !== "CLEAR" || profile.pep === "MATCH") return "BLOCK";
...
export function nextCaseStatus(current: ComplianceCaseStatus, next: ComplianceCaseStatus) {
  const allowed: Record<ComplianceCaseStatus, ComplianceCaseStatus[]> = {
    OPEN: ["REVIEW", "ESCALATED", "CLOSED"],
    REVIEW: ["ESCALATED", "CLEARED", "CLOSED"],
    ESCALATED: ["REVIEW", "CLEARED", "CLOSED"],
    CLEARED: ["CLOSED"],
    CLOSED: [],
  };
